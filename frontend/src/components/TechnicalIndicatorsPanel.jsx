...
export default function TechnicalIndicatorsPanel({ metricsData }) {
  ...
  const rsiColor = rsiVal >= 70 ? 'text-[#EB5B5B]' : rsiVal <= 30 ? 'text-[#00D09C]' : 'text-[#2563EB]';
  const macdPositive = ind.macd_histogram >= 0;

  return (
    <div className="p-4 sm:p-5 rounded-lg border border-slate-200 bg-white shadow-sm flex flex-col gap-4 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 border-b border-slate-200 pb-3">
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          <span className="text-[11px] text-slate-500 uppercase font-bold font-mono">Composite:</span>
          <span
            className={`text-xs font-mono font-extrabold px-2.5 py-1 rounded-md border ${
              q.verdict.includes('Buy')
                ? 'bg-[#00D09C]/15 text-[#047857] border-[#00D09C]/30'
                : q.verdict.includes('Sell')
                ? 'bg-[#EB5B5B]/15 text-[#DC2626] border-[#EB5B5B]/30'
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-md p-3.5 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-700 font-mono">14-Day RSI</span>
            <span className="text-[10px] font-mono uppercase px-2 py-0.5 rounded bg-slate-200 text-slate-600 font-bold">
              {ind.rsi_status}
            </span>
          </div>
          ...
          {/* RSI Visual Meter */}
          <div>
            <div className="w-full h-1.5 rounded-full bg-slate-200 relative overflow-hidden flex">
            ...
            <div className="relative w-full h-1.5 mt-0.5">
              <div
                className="absolute top-0 w-2 h-2 -ml-1 rounded-full bg-slate-900 shadow-sm"
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-md p-3.5 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-700 font-mono">MACD (12, 26, 9)</span>
            <span
          ...
          <div className="my-2 grid grid-cols-3 gap-1 text-center font-mono">
            <div className="bg-white p-1.5 rounded border border-slate-200">
              ...
              <div className="text-xs font-bold text-[#2563EB] tabular-nums">{ind.macd_line.toFixed(2)}</div>
            </div>
            <div className="bg-white p-1.5 rounded border border-slate-200">
              <div className="text-[9px] text-slate-500 uppercase font-bold">Signal</div>
              <div className="text-xs font-bold text-[#D97706] tabular-nums">{ind.macd_signal.toFixed(2)}</div>
            </div>
            <div className="bg-white p-1.5 rounded border border-slate-200">
              <div className="text-[9px] text-slate-500 uppercase font-bold">Hist</div>
              <div className={`text-xs font-bold tabular-nums ${macdPositive ? 'text-[#047857]' : 'text-[#DC2626]'}`}>
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-md p-3.5 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-700 font-mono">Bollinger Bands (20, 2)</span>
            <span className="text-[10px] font-mono font-bold text-[#2563EB]">Width: {bb.bandwidth.toFixed(1)}%</span>
          </div>

          <div className="my-2 space-y-1 text-xs font-mono">
            <div className="flex justify-between text-slate-500">
              <span>Upper:</span>
              <span className="text-slate-900 font-bold tabular-nums">{sym}{bb.upper.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-slate-500">
              <span>Mid (SMA20):</span>
              <span className="text-slate-900 font-bold tabular-nums">{sym}{bb.middle.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-slate-500">
              <span>Lower:</span>
              <span className="text-slate-900 font-bold tabular-nums">{sym}{bb.lower.toFixed(2)}</span>
            </div>
          </div>

          <div className="text-[10px] text-[#2563EB] font-mono font-bold bg-[#2563EB]/10 border border-[#2563EB]/25 px-2 py-0.5 rounded text-center truncate">
            {bb.status}
          </div>
        </div>
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-md p-3.5 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-700 font-mono">30D Annual Volatility</span>
            <span
              className={`text-[10px] font-mono uppercase px-2 py-0.5 rounded border font-bold ${
          ...
          <div className="flex items-center justify-between text-[11px] font-mono pt-1.5 border-t border-slate-200 font-bold">
            <span className="text-[#047857]">Support: {sym}{ind.support_level.toFixed(2)}</span>
            <span className="text-[#DC2626]">Resist: {sym}{ind.resistance_level.toFixed(2)}</span>
          </div>
        </div>
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-md p-3.5 flex items-start gap-2.5">
        <ShieldCheck className="w-5 h-5 text-[#047857] shrink-0 mt-0.5" />
        <div className="text-xs text-slate-700 leading-relaxed font-mono">
          <span className="font-bold text-slate-900 mr-2">Quantitative Summary:</span>

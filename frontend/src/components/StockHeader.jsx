...
export default function StockHeader({ metricsData, loading, onSelectTicker }) {
  ...
  if (loading && !metricsData) {
    return (
      <div className="p-4 sm:p-5 rounded-lg border border-slate-200 bg-white shadow-sm">
  ...
  return (
    <div className="p-4 sm:p-5 rounded-lg border border-slate-200 bg-white shadow-sm flex flex-col gap-3.5 transition-colors">
      ...
      <div className="flex items-center justify-between gap-2 overflow-x-auto no-scrollbar pb-1 border-b border-slate-200">
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-[11px] font-mono text-slate-500 uppercase font-semibold">Quick Watch:</span>
          {quickPills.map((tkr) => (
            <button
              key={tkr}
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            {isConverted && (
              <span className="text-[11px] font-mono text-[#2563EB] bg-[#2563EB]/10 border border-[#2563EB]/25 px-2 py-0.5 rounded-md flex items-center gap-1">
          ...
          <div className="flex items-baseline gap-3 flex-wrap">
            <span className="text-3xl sm:text-4xl font-bold font-mono text-slate-900 tabular-nums tracking-tight">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
          ...
          {/* 52-Week Range Bar */}
          <div className="w-full sm:w-48 flex flex-col gap-1 bg-slate-50 p-2 rounded-md border border-slate-200">
            <div className="flex justify-between text-[10px] font-mono text-slate-500">
              <span>L: {activeSym}{displayLow52.toFixed(1)}</span>
              <span>H: {activeSym}{displayHigh52.toFixed(1)}</span>
            </div>
            <div className="w-full h-1.5 rounded-full bg-slate-200 relative overflow-hidden">
              <div
          ...
          <div className="grid grid-cols-3 gap-2 text-center font-mono">
            <div className="bg-slate-50 border border-slate-200 p-2 rounded-md">
              <div className="text-[10px] text-slate-500 uppercase font-bold">Mkt Cap</div>
              <div className="text-xs font-bold text-slate-900 tabular-nums">{formatLargeNum(m.market_cap)}</div>
            </div>
            <div className="bg-slate-50 border border-slate-200 p-2 rounded-md">
              <div className="text-[10px] text-slate-500 uppercase font-bold">P/E</div>
              <div className="text-xs font-bold text-slate-900 tabular-nums">{m.pe_ratio ? m.pe_ratio.toFixed(1) : 'N/A'}</div>
            </div>
            <div className="bg-slate-50 border border-slate-200 p-2 rounded-md">
              <div className="text-[10px] text-slate-500 uppercase font-bold">Beta</div>
              <div className="text-xs font-bold text-slate-900 tabular-nums">{m.beta ? m.beta.toFixed(2) : '1.00'}</div>
            </div>
          </div>

          {/* Quant Verdict */}
          <div className="p-2 rounded-md bg-slate-50 border border-slate-200 text-center flex flex-col justify-center">
            <div className="text-[10px] uppercase font-bold text-slate-500 font-mono">Signal</div>
            <span
